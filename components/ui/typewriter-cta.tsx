...
export default function TypewriterCTA() {
  ...
  const handleClick = () => {
    ...
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('ctaContext', JSON.stringify({
    ...
    >
      <Link href="/contact" onClick={handleClick}>
        <Button
          ...
          className="
            relative min-h-[60px] px-4 sm:px-8 py-4
            ...
            group
            w-full max-w-[90vw] sm:max-w-none
          "
        >
          <span className="flex items-center gap-2 sm:gap-3">
            <span className="block sm:inline-block min-w-0 sm:min-w-[250px] text-left whitespace-normal sm:whitespace-nowrap">
              {displayedText}
              <motion.span
                animate={{ opacity: [1, 0] }}
